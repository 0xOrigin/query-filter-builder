...
/**
 * A {@link FilterOperator} implementation that handles the 'not equals' operation.
 */
public final class NotEquals implements FilterOperator {
    ...
    public <T extends Comparable<? super T> & Serializable> Optional<Predicate> apply(Expression<T> expression, CriteriaBuilder cb, List<T> values, FilterErrorWrapper filterErrorWrapper) {
        ...
        return Optional.ofNullable(cb.notEqual(expression, values.get(0)));
    }
