...
/**
 * A {@link FilterOperator} implementation that handles the 'equals' operation.
 */
public final class Equals implements FilterOperator {
    ...
    public <T extends Comparable<? super T> & Serializable> Optional<Predicate> apply(Expression<T> expression, CriteriaBuilder cb, List<T> values, FilterErrorWrapper filterErrorWrapper) {
        ...
        return Optional.ofNullable(cb.equal(expression, values.get(0)));
    }
