...
class NotBetweenTest {
    ...
    void apply_validListWithTwoElements_returnsPredicate() {
        ...
        when(criteriaBuilder.between(expression, values.get(0), values.get(1))).thenReturn(betweenPredicate);
        when(criteriaBuilder.not(betweenPredicate)).thenReturn(predicate);

        Optional<Predicate> result = notBetween.apply(expression, criteriaBuilder, values, filterErrorWrapper);

        assertThat(result).isPresent().contains(predicate);
        verify(criteriaBuilder).between(expression, values.get(0), values.get(1));
        verify(criteriaBuilder).not(betweenPredicate);
    ...
    void apply_listWithWrongSize_addsErrorAndReturnsEmptyOptional() {
        List<Integer> values = List.of(1, 10, 100);
        Expression<Integer> expression = mock(Expression.class);
        ...
        when(localizationService.getMessage(Operator.NOT_BETWEEN.getValue())).thenReturn("not between");
        when(localizationService.getMessage(MessageKey.VALUE_MUST_EXACTLY_TWO_ELEMENTS.getCode(), "not between"))
            .thenReturn("Value must have exactly two elements for not between operator");

        Optional<Predicate> result = notBetween.apply(expression, criteriaBuilder, values, filterErrorWrapper);

        assertThat(result).isEmpty();
        verify(filterErrorWrapper.bindingResult()).addError(any());
        verify(localizationService).getMessage(MessageKey.VALUE_MUST_EXACTLY_TWO_ELEMENTS.getCode(), "not between");
        verifyNoInteractions(criteriaBuilder);
    }

    @Test
    void getOperatorConstant_returnsNotBetweenOperator() {
        assertThat(notBetween.getOperatorConstant()).isEqualTo(Operator.NOT_BETWEEN);
    }
}
