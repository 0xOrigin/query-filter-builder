...
class LessThanOrEqualTest {
    ...
    void apply_validListWithSingleElement_returnsPredicate() {
        ...
        when(criteriaBuilder.lessThanOrEqualTo(expression, values.get(0))).thenReturn(predicate);

        Optional<Predicate> result = lessThanOrEqual.apply(expression, criteriaBuilder, values, filterErrorWrapper);

        assertThat(result).isPresent().contains(predicate);
        verify(criteriaBuilder).lessThanOrEqualTo(expression, values.get(0));
    ...
    @Test
    void getOperatorConstant_returnsLessThanOrEqualOperator() {
        assertThat(lessThanOrEqual.getOperatorConstant()).isEqualTo(Operator.LTE);
    }
}
