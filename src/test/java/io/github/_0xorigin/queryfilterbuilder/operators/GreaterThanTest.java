...
class GreaterThanTest {
    ...
    void apply_validListWithSingleElement_returnsPredicate() {
        ...
        when(criteriaBuilder.greaterThan(expression, values.get(0))).thenReturn(predicate);

        Optional<Predicate> result = greaterThan.apply(expression, criteriaBuilder, values, filterErrorWrapper);

        assertThat(result).isPresent().contains(predicate);
        verify(criteriaBuilder).greaterThan(expression, values.get(0));
    ...
    @Test
    void getOperatorConstant_returnsGreaterThanOperator() {
        assertThat(greaterThan.getOperatorConstant()).isEqualTo(Operator.GT);
    }
}
