...
class GreaterThanTest {
    ...
    void apply_WithNullValues_ReturnsPredicate() {
        ...
        Predicate result = greaterThan.apply(path, cb, values, errorWrapper);
    ...
    void apply_WithNumericValue_ReturnsPredicate() {
        ...
        assertNotNull(result);
        verify(cb).greaterThan((Expression<Integer>) path, 10);
        assertFalse(errorWrapper.getBindingResult().hasErrors());
    }
    ...
    void apply_WithLocalDateValue_ReturnsPredicate() {
        ...
        when(cb.greaterThan(any(), any(Date.class))).thenReturn(predicate);

        Predicate result = greaterThan.apply(path, cb, values, errorWrapper);

        assertNotNull(result);
        verify(cb).greaterThan(any(), eq(Date.from(date.atStartOfDay(ZoneId.systemDefault()).toInstant())));
        assertFalse(errorWrapper.getBindingResult().hasErrors());
    }
    ...
    void apply_WithLocalDateTimeValue_ReturnsPredicate() {
        ...
        when(cb.greaterThan(any(), any(Timestamp.class))).thenReturn(predicate);

        Predicate result = greaterThan.apply(path, cb, values, errorWrapper);

        assertNotNull(result);
        verify(cb).greaterThan(any(), eq(Timestamp.from(dateTime.atZone(ZoneId.systemDefault()).toInstant())));
        assertFalse(errorWrapper.getBindingResult().hasErrors());
    }
    ...
    void apply_WithLocalTimeValue_ReturnsPredicate() {
        ...
        when(cb.greaterThan(any(), any(Time.class))).thenReturn(predicate);

        errorWrapper = new ErrorWrapper(bindingResult, new FilterWrapper("", "", Operator.GT, values));
        Predicate result = greaterThan.apply(path, cb, values, errorWrapper);

        assertNotNull(result);
        verify(cb).greaterThan(any(), eq(Time.valueOf(time)));
        assertFalse(errorWrapper.getBindingResult().hasErrors());
    }
    ...
    void apply_WithTimestampValue_ReturnsPredicate() {
        ...
        assertNotNull(result);
        verify(cb).greaterThan(any(), eq(timestamp));
        assertFalse(errorWrapper.getBindingResult().hasErrors());
    }
    ...
    void apply_WithStringValue_ReturnsPredicate() {
        ...
        when(path.as(Comparable.class)).thenReturn((Expression<Comparable>) path);
        when(cb.greaterThan(any(), any(String.class))).thenReturn(predicate);

        Predicate result = greaterThan.apply(path, cb, values, errorWrapper);

        assertNotNull(result);
        verify(cb).greaterThan((Expression<String>) path, "test");
        assertFalse(errorWrapper.getBindingResult().hasErrors());
    }
