...
class LessThanTest {
    ...
    void apply_WithNullValues_ReturnsPredicate() {
        ...
        Predicate result = lessThan.apply(path, cb, values, errorWrapper);
    ...
    void apply_WithNumericValue_ReturnsPredicate() {
        ...
        assertNotNull(result);
        verify(cb).lessThan((Expression<Integer>) path, 10);
        assertFalse(errorWrapper.getBindingResult().hasErrors());
    }
    ...
    void apply_WithLocalDateValue_ReturnsPredicate() {
        ...
        when(cb.lessThan(any(), any(Date.class))).thenReturn(predicate);

        Predicate result = lessThan.apply(path, cb, values, errorWrapper);

        assertNotNull(result);
        verify(cb).lessThan(any(), eq(Date.from(date.atStartOfDay(ZoneId.systemDefault()).toInstant())));
        assertFalse(errorWrapper.getBindingResult().hasErrors());
    }
    ...
    void apply_WithLocalDateTimeValue_ReturnsPredicate() {
        ...
        when(cb.lessThan(any(), any(Timestamp.class))).thenReturn(predicate);

        Predicate result = lessThan.apply(path, cb, values, errorWrapper);

        assertNotNull(result);
        verify(cb).lessThan(any(), eq(Timestamp.from(dateTime.atZone(ZoneId.systemDefault()).toInstant())));
        assertFalse(errorWrapper.getBindingResult().hasErrors());
    }
    ...
    void apply_WithLocalTimeValue_ReturnsPredicate() {
        ...
        when(cb.lessThan(any(), any(Time.class))).thenReturn(predicate);

        errorWrapper = new ErrorWrapper(bindingResult, new FilterWrapper("", "", Operator.LT, values));
        Predicate result = lessThan.apply(path, cb, values, errorWrapper);

        assertNotNull(result);
        verify(cb).lessThan(any(), eq(Time.valueOf(time)));
        assertFalse(errorWrapper.getBindingResult().hasErrors());
    }
    ...
    void apply_WithTimestampValue_ReturnsPredicate() {
        ...
        assertNotNull(result);
        verify(cb).lessThan(any(), eq(timestamp));
        assertFalse(errorWrapper.getBindingResult().hasErrors());
    }
    ...
    void apply_WithStringValue_ReturnsPredicate() {
        ...
        when(path.as(Comparable.class)).thenReturn((Expression<Comparable>) path);
        when(cb.lessThan(any(), any(String.class))).thenReturn(predicate);

        Predicate result = lessThan.apply(path, cb, values, errorWrapper);

        assertNotNull(result);
        verify(cb).lessThan((Expression<String>) path, "test");
        assertFalse(errorWrapper.getBindingResult().hasErrors());
    }
