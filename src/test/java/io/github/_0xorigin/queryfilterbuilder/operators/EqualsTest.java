...
class EqualsTest {
    ...
    void apply_validListWithSingleElement_returnsPredicate() {
        ...
        when(criteriaBuilder.equal(expression, values.get(0))).thenReturn(predicate);

        Optional<Predicate> result = equals.apply(expression, criteriaBuilder, values, filterErrorWrapper);

        assertThat(result).isPresent().contains(predicate);
        verify(criteriaBuilder).equal(expression, values.get(0));
    ...
    @Test
    void getOperatorConstant_returnsEqualsOperator() {
        assertThat(equals.getOperatorConstant()).isEqualTo(Operator.EQ);
    }
}
