...
class NotEqualsTest {
    ...
    void apply_validListWithSingleElement_returnsPredicate() {
        ...
        when(criteriaBuilder.notEqual(expression, values.get(0))).thenReturn(predicate);

        Optional<Predicate> result = notEquals.apply(expression, criteriaBuilder, values, filterErrorWrapper);

        assertThat(result).isPresent().contains(predicate);
        verify(criteriaBuilder).notEqual(expression, values.get(0));
    ...
    @Test
    void getOperatorConstant_returnsNotEqualsOperator() {
        assertThat(notEquals.getOperatorConstant()).isEqualTo(Operator.NEQ);
    }
}
